...
var playedGames = 0
var playerAWins, playerBWins = 0, 0
var plotCardGraph bool

func main() {
	flag.IntVar(&numberOfCards, "n", 32, "Number of Cards to play with")
	flag.IntVar(&numberOfGames, "g", 1, "Number of games to play")
	flag.BoolVar(&plotCardGraph, "plotCardGraph", false, "Plot Card Graph")
	flag.Parse()
	fmt.Printf("Playing %d games with %d cards\n", numberOfGames, numberOfCards)

	var staple []model.Card

	for playedGames < numberOfGames {
		playedGames++
		staple = nil
	...
	fmt.Printf("Player A wins: %d Player B wins: %d \n", playerAWins, playerBWins)
